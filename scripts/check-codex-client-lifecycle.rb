...
MANIFEST = ROOT.join("client-lifecycle/codex-native-guidance-v1.json")
TEMPLATE = ROOT.join("templates/client-guidance/codex/AGENTS.md")

def run_check(target:, version: "0.149.0-alpha.4.1", available: true, os: "macos", arch: "aarch64", as_of: "2026-08-27")
...
codex = public_manifest.fetch("client_support").find { |entry| entry["client"] == "Codex" }
abort("public CI-4 claim missing") unless codex && codex.fetch("lifecycle_maintenance") == {
  "level" => "L4",
  "manifest" => "client-lifecycle/codex-native-guidance-v1.json",
...
Dir.mktmpdir("impresari-codex-client-lifecycle-") do |directory|
  ...
  cases = {
    ...
    "missing" => run_check(target: root.join("missing-AGENTS.md")),
  }
  changed = root.join("changed-AGENTS.md")
  changed.write(TEMPLATE.read + "\nchanged\n")
  cases["changed"] = run_check(target: changed)
  unowned = root.join("unowned-AGENTS.md")
  unowned.write("unowned\n")
  cases["unowned"] = run_check(target: unowned)

  expected = {
    "compatible" => "compatible", "stale_evidence" => "stale_evidence",
    "unsupported" => "unsupported", "unknown" => "unknown",
    "client_unavailable" => "degraded", "missing" => "degraded",
    "changed" => "degraded", "unowned" => "degraded",
  }
...
malformed = ROOT.join("client-lifecycle/malformed-codex-test-only.json")
begin
  File.write(malformed, "{not-json")
  _stdout, _stderr, status = Open3.capture3(RbConfig.ruby, CHECKER.to_s, "--manifest", malformed.to_s, "--target", TEMPLATE.to_s, "--client-version", "0.149.0-alpha.4.1", "--client-available", "yes", "--os", "macos", "--arch", "aarch64", "--as-of", "2026-08-27")
  abort("malformed manifest was accepted") if status.success?
ensure
  File.delete(malformed) if malformed.exist?
end

puts "Codex client lifecycle checks passed: v2 guidance identity and 8 deterministic health states"
