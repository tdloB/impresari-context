...
MANIFEST = ROOT.join("client-lifecycle/vscode-copilot-guidance-delivery-v1.json")
TEMPLATE = ROOT.join("templates/client-guidance/copilot/impresari-context.instructions.md")

def run_check(target:, version: "1.134.0", available: true, os: "macos", arch: "aarch64", as_of: "2026-08-29")
...
abort("L3 delivery contract drift") unless manifest.fetch("delivery_contract") == {
  "level" => "L3",
  "mode" => "ask",
...
vscode = public_manifest.fetch("client_support").find { |entry| entry["client"] == "VS Code Copilot" }
abort("public CI-4 claim missing") unless vscode && vscode.fetch("lifecycle_maintenance") == {
  "level" => "L4",
  "manifest" => "client-lifecycle/vscode-copilot-guidance-delivery-v1.json",
...
Dir.mktmpdir("impresari-vscode-client-lifecycle-") do |directory|
  ...
  cases = {
    ...
    "stale_evidence" => run_check(target: owned, as_of: "2026-11-28"),
  ...
  abort("delivery contract check missing") unless cases.fetch("compatible").fetch("checks").include?("delivery_contract_bound")
...
malformed = ROOT.join("client-lifecycle/malformed-vscode-test-only.json")
drifted = ROOT.join("client-lifecycle/drifted-vscode-test-only.json")
begin
  File.write(malformed, "{not-json")
  _stdout, _stderr, status = Open3.capture3(RbConfig.ruby, CHECKER.to_s, "--manifest", malformed.to_s, "--target", TEMPLATE.to_s, "--client-version", "1.134.0", "--client-available", "yes", "--os", "macos", "--arch", "aarch64", "--as-of", "2026-08-29")
  abort("malformed manifest was accepted") if status.success?
  drifted_manifest = manifest.dup
  drifted_manifest["delivery_contract"] = manifest.fetch("delivery_contract").merge("mode" => "agent")
  drifted.write(JSON.generate(drifted_manifest))
  _stdout, _stderr, status = Open3.capture3(RbConfig.ruby, CHECKER.to_s, "--manifest", drifted.to_s, "--target", TEMPLATE.to_s, "--client-version", "1.134.0", "--client-available", "yes", "--os", "macos", "--arch", "aarch64", "--as-of", "2026-08-29")
  abort("drifted delivery contract was accepted") if status.success?
ensure
  File.delete(malformed) if malformed.exist?
  File.delete(drifted) if drifted.exist?
end

puts "VS Code Copilot client lifecycle checks passed: L2/L3 identities and 8 deterministic health states"
