...
MANIFEST = ROOT.join("client-lifecycle/claude-code-native-guidance-v1.json")
TEMPLATE = ROOT.join("templates/client-guidance/claude/SKILL.md")

def run_check(target:, version: "2.1.241", available: true, os: "macos", arch: "aarch64", as_of: "2026-08-27")
...
claude = public_manifest.fetch("client_support").find { |entry| entry["client"] == "Claude Code" }
abort("public CI-4 claim missing") unless claude && claude.fetch("lifecycle_maintenance") == {
  "level" => "L4",
  "manifest" => "client-lifecycle/claude-code-native-guidance-v1.json",
...
Dir.mktmpdir("impresari-claude-client-lifecycle-") do |directory|
  ...
  cases = {
    ...
    "missing" => run_check(target: root.join("missing-skill.md")),
  }
  changed = root.join("changed-skill.md")
  changed.write(TEMPLATE.read + "\nchanged\n")
  cases["changed"] = run_check(target: changed)
  unowned = root.join("unowned-skill.md")
...
malformed = ROOT.join("client-lifecycle/malformed-claude-test-only.json")
begin
  File.write(malformed, "{not-json")
  _stdout, _stderr, status = Open3.capture3(RbConfig.ruby, CHECKER.to_s, "--manifest", malformed.to_s, "--target", TEMPLATE.to_s, "--client-version", "2.1.241", "--client-available", "yes", "--os", "macos", "--arch", "aarch64", "--as-of", "2026-08-27")
  abort("malformed manifest was accepted") if status.success?
ensure
  File.delete(malformed) if malformed.exist?
end

puts "Claude Code client lifecycle checks passed: manifest identity and 8 deterministic health states"
