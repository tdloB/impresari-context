...
| --- | --- | --- |
| CI-1 managed connections | Shared render/inspect/validate/install/remove capability and deterministic fixture coverage exist. Codex and Claude Code L1 local evidence are complete for their recorded macOS/client-version scopes. Cursor and Copilot remain L0 until their independent admission records are complete. | Maintain Codex and Claude Code after every upstream change; complete only user-consented, disposable client lifecycle evidence where required for Cursor and Copilot. |
