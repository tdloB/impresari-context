...
| CI-3 guided delivery | CI-3a implements a strict client-neutral intent/receipt contract. CI-3b admits Codex App Server `0.150.0-alpha.8` at recorded-scope L3. CI-3c independently admits Copilot CLI `1.0.80` through two zero-tool programmatic-prompt deliveries. CI-3d independently admits Claude Code `2.1.241` through two safe-mode streamed deliveries. CI-3e independently admits Cursor Agent `2026.08.25-3e8eec8` through two ask-mode, sandboxed, zero-tool stream deliveries in trusted empty disposable runtimes. CI-3f independently admits VS Code Copilot `1.134.0` on macOS arm64 through two Ask-mode chat-CLI deliveries from empty disposable runtimes, with exact packet acknowledgments, immutable source, existing authentication used in place, cleanup, and no provider-delivery inference from launcher exit. | Maintain all exact scopes after upstream changes. Copilot `1.0.81` remains excluded because its model request retained built-in tool schemas despite exclusion flags. VS Code L3 must be withdrawn if its prompt/stdin/UI behavior changes. |
| CI-4 lifecycle maintenance | A versioned manifest/receipt contract and one-shot read-only checker are implemented for exact Codex CLI `0.149.0-alpha.4.1` native-guidance v2, GitHub Copilot CLI `1.0.80` native-guidance v3, Claude Code `2.1.241` native-guidance v2, Cursor Agent `2026.08.11-e8db854` native-guidance v2, and VS Code Copilot `1.134.0` native-guidance v3 plus operator-confirmed Ask-mode L3 contracts on macOS aarch64. Independent deterministic gates cover compatible, stale, unsupported, unrecorded-version, client-unavailable, missing, changed, unowned, malformed, source-immutability, exact-removal, and VS Code delivery-contract binding states. Codex L4 is deliberately limited to its recorded L2 artifact and does not maintain the separate App Server L3 scope. | Revalidate each recorded scope before its freshness window expires or after an upstream lifecycle change. Withdraw VS Code's L3/L4 claim after prompt/stdin/UI drift until new rehearsals pass. |
...
| --- | --- | --- | --- |
| Codex | L1/L2/L3/L4 | Maintain exact scopes | L1 remains admitted for Codex CLI `0.149.0-alpha.4.1`, L2 for its separately recorded `AGENTS.md` v2 smoke, and L3 independently for App Server `0.150.0-alpha.8` on macOS arm64. L4 is limited to an explicit source-free check of the exact v2 `AGENTS.md` and its recorded `0.149.0-alpha.4.1` scope; it performs no client discovery or repair and does not maintain the separate L3 version. L3 requires explicit preview/apply, a dedicated operator-authenticated home, an ephemeral read-only/no-network thread, exact packet binding, and no added authority. |
| Claude Code | L1/L2/L3/L4 | Maintain exact scopes | L1 is admitted only for Claude Code CLI `2.1.241` on macOS aarch64. L2 is separately admitted for the project skill at `.claude/skills/impresari-context/SKILL.md`; its model-directed smoke is not a repeatability guarantee. L3 is independently admitted through two explicit preview/apply deliveries using a zero-tool, no-session, safe-mode print process with exact prompt replay and without installing hooks. L4 is limited to an explicit source-free check of that exact v2 skill and recorded scope; it performs no client discovery or repair. |
| Cursor | L1/L2/L3/L4 | Maintain exact scopes | L1 is admitted only for Cursor Agent CLI `3.17.8` (`2026.08.11-e8db854`) on macOS aarch64: explicit project scope, malformed-client rejection, native enable/list-tools/disable, bounded guarded Agent-mode lifecycle, packet equivalence, and source immutability are recorded. L2 is separately admitted for the opt-in `.cursor/rules/impresari-context.mdc` v2 rule through an isolated Agent-mode smoke. L3 is independently admitted for Cursor Agent `2026.08.25-3e8eec8` through two explicit ask-mode, sandboxed stream deliveries in trusted empty disposable runtimes, with exact packet binding, prompt/cwd evidence, zero tool calls, immutable source, in-place authentication, and cleanup. L4 is limited to an explicit source-free check of the exact v2 rule and its recorded `2026.08.11-e8db854` scope; it performs no client discovery or repair. Conversational rule selection remains non-deterministic; user-owned IDE enablement remains explicit. |
...
| VS Code Copilot extension host | L1/L2/L3/L4 | Maintain exact scopes | L1 and L2 are admitted only for VS Code `1.134.0` on macOS arm64. L1 records strict workspace `.vscode/mcp.json` configuration, explicit user trust/start, server discovery, bounded lifecycle, source immutability, and exact removal. L2 separately records the active v3 instruction, successful same-session packet build/resolve, Copilot-compatible flat tool schema, source immutability, exact-owned cleanup, and hosted CI. L3 is independently admitted through two explicit Ask-mode chat-CLI deliveries using a bounded positional prompt and exact stdin context from empty disposable runtimes. Because the UI-opening surface has no machine-readable response stream, final delivery requires exact operator-observed packet acknowledgment and is never inferred from launcher exit. L4 is an explicit source-free read-only check binding the exact v3 guidance, recorded L3 protocol/evidence, client/platform scope, and freshness window; it performs no client discovery, process launch, repair, or monitoring. It is distinct from the Copilot CLI; the root `.mcp.json` Agent Host surface remains generic/unadmitted. Conversational tool selection remains non-deterministic. |
