...
| CI-2 native guidance | Versioned Codex, Claude Code, Cursor, and Copilot templates; deterministic render/inspect/validate/install/remove lifecycle; static authority-boundary checks; and exact legacy-removal support are implemented. Codex, Claude Code, Cursor, and GitHub Copilot CLI have recorded v2 L2 scopes. Copilot v3 is in validation: its CLI revalidation and VS Code extension-host packet build/resolve smoke are distinct gates. Codex has no separate deterministic instruction-source signal from its App Server. | Complete the v3 CLI revalidation and the user-consented disposable VS Code L2 record before promoting v3 or the VS Code surface. |
...
| GitHub Copilot CLI | L1/L2 (v2 record); v3 validation pending | L3 where native surfaces permit | L1 is admitted only for Copilot CLI `1.0.80` on macOS aarch64: explicit project scope, malformed-client rejection, isolated native `list/get` discovery after exact workspace trust, bounded prompt-mode packet equivalence, and exact owned-entry/trust removal are recorded. L2 is recorded for the opt-in v2 instruction. The v3 instruction adds canonical request ergonomics and requires a new isolated CLI smoke before it replaces that template claim. Conversational tool selection remains non-deterministic. |
| VS Code Copilot extension host | L1 | L2, then L3 where native surfaces permit | L1 is admitted only for VS Code `1.134.0` on macOS arm64: strict workspace `.vscode/mcp.json` configuration, explicit user trust/start, visible server discovery, bounded session-tool use, source immutability, exact owned removal, and hosted CI are recorded. It is distinct from the Copilot CLI. The root `.mcp.json` Agent Host surface remains generic/unadmitted. The new v3 guidance/build-schema ergonomics runner must show an active instruction plus successful same-session packet build/resolve before L2 is claimed. |
