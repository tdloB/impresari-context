...
| CI-3 guided delivery | CI-3a implements a strict client-neutral intent/receipt contract. CI-3b adds an experimental Codex-only preview/apply adapter for CLI `0.150.0-alpha.8` on macOS aarch64: exact preview rehydration, complete initialization, authentication preflight, ephemeral read-only/no-network App Server session, authority denial, bounded receipt, and temporary-runtime cleanup. The isolated runtime now declines before packet delivery with `codex_auth_unavailable`, so no L3 promotion is claimed. | Design an explicit authenticated-host boundary without silently copying or persisting credentials, then require two successful lifecycle records under the exact scope before L3 admission. Every other client still needs independent consent, removal, redaction, degradation, source-immutability, and live lifecycle evidence. |
...
| --- | --- | --- | --- |
| Codex | L1/L2 | L3 where native surfaces permit | L1 remains admitted for the recorded Codex CLI `0.149.0-alpha.4.1` scope, and L2 for its separately recorded `AGENTS.md` v2 smoke. CI-3b is structurally revalidated for App Server `0.150.0-alpha.8`; it completes the required handshake and fails before packet delivery when the isolated runtime lacks authentication. L3 awaits an approved authenticated-host design and two successful live records. |
...
| GitHub Copilot CLI | L1/L2/L4 | L3 where native surfaces permit | L1 is admitted only for Copilot CLI `1.0.80` on macOS aarch64: explicit project scope, malformed-client rejection, isolated native `list/get` discovery after exact workspace trust, bounded prompt-mode packet equivalence, and exact owned-entry/trust removal are recorded. L2 is recorded for the opt-in v3 instruction after a separate isolated revalidation with native discovery, four-tool lifecycle, direct packet equivalence, source immutability, and exact trust/configuration removal. L4 is limited to an explicit source-free check of that exact v3 artifact and recorded scope; it performs no client discovery or repair. Conversational tool selection remains non-deterministic. |
