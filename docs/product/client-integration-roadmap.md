...
| CI-2 native guidance | Versioned Codex, Claude Code, Cursor, and Copilot templates; deterministic render/inspect/validate/install/remove lifecycle; static authority-boundary checks; and exact v1 legacy-removal support are implemented. Claude Code L2 is admitted for recorded CLI `2.1.241` / macOS aarch64 scope through an isolated project-skill smoke. Codex L2 is admitted for recorded CLI `0.149.0-alpha.4.1` / macOS aarch64 scope through an isolated project-instructions live smoke, with no separate deterministic instruction-source signal from its App Server. | Admit Cursor and GitHub Copilot L2 separately with their own client/version/OS native-surface evidence; revalidate Codex if its instruction-source reporting changes. |
...
| --- | --- | --- | --- |
| Codex | L1/L2 | L3 where native surfaces permit | L1 is admitted only for Codex CLI `0.149.0-alpha.4.1` on macOS aarch64: explicit user-home kit, malformed-client rejection, deterministic lifecycle, packet equivalence, and exact removal are recorded. L2 is separately admitted for a stand-alone project `AGENTS.md` v2 artifact through a disposable live CLI smoke. The current App Server returned no instruction-source record, so L2 does not claim deterministic native-guidance discovery or repeatable tool selection. |
