...
| --- | --- | --- |
| CI-1 managed connections | Shared render/inspect/validate/install/remove capability and deterministic fixture coverage exist. Codex, Claude Code, and Cursor L1 local evidence are complete for their recorded macOS/client-version scopes. Copilot remains L0 until its CLI and VS Code surfaces independently complete admission. | Maintain the three admitted clients after upstream changes; complete only user-consented, disposable client lifecycle evidence for GitHub Copilot CLI, then the distinct VS Code Copilot surface. |
