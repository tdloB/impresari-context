...
| CI-4 lifecycle maintenance | A versioned manifest/receipt contract and one-shot read-only checker are implemented for exact GitHub Copilot CLI `1.0.80` native-guidance v3, Claude Code `2.1.241` native-guidance v2, Cursor Agent `2026.08.11-e8db854` native-guidance v2, and VS Code Copilot `1.134.0` native-guidance v3 plus operator-confirmed Ask-mode L3 contracts on macOS aarch64. Independent deterministic gates cover compatible, stale, unsupported, unrecorded-version, client-unavailable, missing, changed, unowned, malformed, source-immutability, exact-removal, and VS Code delivery-contract binding states. | Revalidate each recorded scope before its freshness window expires or after an upstream lifecycle change. Withdraw VS Code's L3/L4 claim after prompt/stdin/UI drift until new rehearsals pass. |
