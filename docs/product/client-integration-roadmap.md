...
| CI-3 guided delivery | CI-3a implements a strict client-neutral intent/receipt contract. CI-3b admits Codex App Server `0.150.0-alpha.8` at recorded-scope L3. CI-3c independently admits Copilot CLI `1.0.80` through exact preview rehydration, explicit isolated authentication, two zero-tool programmatic-prompt deliveries, immutable source, bounded receipts, and runtime cleanup. | Maintain both exact scopes after upstream changes. Copilot `1.0.81` is not admitted because its model request retained built-in tool schemas despite exclusion flags. Every other client still needs independent evidence before L3. |
