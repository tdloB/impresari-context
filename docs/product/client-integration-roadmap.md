...
| CI-3 guided delivery | CI-3a implements a strict client-neutral intent/receipt contract: explicit consent; client/scope/version/lifecycle binding; caller-declared workspace/snapshot identity; canonical planner bytes; stale and malformed no-delivery paths; and no client I/O or added authority. No client delivery adapter is implemented or claimed. | Reassess only stable official lifecycle surfaces one client at a time; a client-specific adapter still needs its own consent, removal, redaction, degradation, source-immutability, and live lifecycle evidence. |
