...
| CI-3 guided delivery | CI-3a implements a strict client-neutral intent/receipt contract. CI-3b admits Codex App Server `0.150.0-alpha.8` at recorded-scope L3. CI-3c independently admits Copilot CLI `1.0.80` through exact preview rehydration, explicit isolated authentication, two zero-tool programmatic-prompt deliveries, immutable source, bounded receipts, and runtime cleanup. CI-3d implements Claude Code `2.1.241` safe-mode preview/apply delivery with exact streamed-input evidence; live admission remains pending explicit provider-disclosure authorization and two successful records. | Finish CI-3d live admission, then maintain all exact scopes after upstream changes. Copilot `1.0.81` is not admitted because its model request retained built-in tool schemas despite exclusion flags. Cursor and VS Code Copilot still need independent evidence before L3. |
...
| Claude Code | L1/L2; L3 implementation pending live admission | L3 through safe-mode programmatic delivery | L1 is admitted only for Claude Code CLI `2.1.241` on macOS aarch64: explicit local scope, malformed-client rejection, native add/get/removal, bounded model-directed lifecycle, packet equivalence, and source immutability are recorded. L2 is separately admitted for the project skill at `.claude/skills/impresari-context/SKILL.md`; its model-directed smoke is not a repeatability guarantee. CI-3d adds explicit preview/apply delivery through a zero-tool, no-session, safe-mode print process without installing hooks. L3 requires two authorized live records before promotion. |
