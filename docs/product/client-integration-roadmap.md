...
| CI-3 guided delivery | CI-3a implements a strict client-neutral intent/receipt contract. CI-3b admits Codex App Server `0.150.0-alpha.8` at recorded-scope L3 on macOS arm64 after exact preview rehydration, complete initialization, explicit dedicated-home authentication, two successful ephemeral read-only/no-network turns, authority denial, immutable source, bounded receipts, and runtime cleanup. | Maintain the exact Codex scope after upstream changes. Every other client still needs independent consent, removal, redaction, degradation, source-immutability, and live lifecycle evidence before L3. |
