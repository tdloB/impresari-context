...
| --- | --- | --- |
| CI-1 managed connections | Shared render/inspect/validate/install/remove capability and deterministic fixture coverage exist. Codex, Claude Code, Cursor, and GitHub Copilot CLI L1 local evidence are complete for their recorded macOS/client-version scopes. VS Code Copilot's extension-host `.vscode/mcp.json` strict-stdio candidate and disposable operator-evidence runner are in implementation, but it is still L0 pending live-client evidence. Portable Agent Host `.mcp.json` remains a separate unadmitted path. Copilot's CLI and VS Code surfaces remain separate claims. | Maintain the four admitted clients after upstream changes; complete only user-consented, disposable lifecycle evidence for the distinct VS Code Copilot extension-host surface. |
...
| GitHub Copilot CLI | L1/L2/L4 | L3 where native surfaces permit | L1 is admitted only for Copilot CLI `1.0.80` on macOS aarch64: explicit project scope, malformed-client rejection, isolated native `list/get` discovery after exact workspace trust, bounded prompt-mode packet equivalence, and exact owned-entry/trust removal are recorded. L2 is separately admitted for the opt-in `.github/instructions/impresari-context.instructions.md` v2 instruction through an isolated Copilot CLI session with custom instructions enabled only for that session. L4 is limited to an explicit source-free check of that exact v2 artifact and recorded scope; it performs no client discovery or repair. Conversational tool selection remains non-deterministic. |
| VS Code Copilot extension host | L1 | L2, then L3 where native surfaces permit | L1 is admitted only for VS Code `1.134.0` on macOS arm64: strict workspace `.vscode/mcp.json` configuration, explicit user trust/start, visible server discovery, bounded session-tool use, source immutability, exact owned removal, and hosted CI are recorded. It is distinct from the Copilot CLI. The root `.mcp.json` Agent Host surface remains generic/unadmitted. No packet-equivalence or conversational-repeatability claim is made; the observed invalid `context_build` request is the next L2 guidance/ergonomics target. |
