...
| ADR-0074 Linux IAR-1B candidate | Exact-host composite candidate passed; broader Linux admission pending | PR 131 job `99197119262` atomically placed one source-free worker under the frozen limits and reproduced the primitive plus resource/lifecycle boundary on kernel `6.17.0-1022-azure`, `x86_64`, Landlock ABI 7. | Exact-host receipt sets `os_confined=true`; no real analyzer or production admission. Multi-kernel and independently admitted architecture evidence remain required. |
