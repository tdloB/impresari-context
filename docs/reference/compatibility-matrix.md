...
| Claude Code | Generic local MCP | A non-mutating local-scope command and read-only JSON validator preserve the fixed local-stdio contract. Claude Code CLI `2.1.241` on macOS aarch64 rejects malformed strict temporary configuration and completed an isolated temporary-config lifecycle with direct packet equivalence and no persistent registration. See the [Phase 1 kit record](../verification/phase-1-claude-code-connection-kit.md). | Not first-class: a user-reviewed local-scope install/remove record remains. Its model-directed lifecycle is honest smoke evidence, not a deterministic prompt-repeatability claim. |
