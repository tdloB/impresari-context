...
| ADR-0074 macOS IAR-1B candidate | Not admitted; IAR-1A remains supported posture | Ad hoc signed App Sandbox host with private App Sandbox XPC service proved bounded synthetic IPC plus selected access-control denials on macOS `26.5.1` arm64 | The documented architecture lacks a hard per-job memory, process-count/tree, and deterministic teardown composition. No real analyzer backend, privileged daemon, private API, VM, production signing, or packaging claim. |
