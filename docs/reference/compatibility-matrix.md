...
| ADR-0074 Linux IAR-1B candidate | x86_64 exact-host composite passed; arm64 evidence pending | PR 131 job `99197119262` atomically placed one source-free worker under the frozen limits and reproduced the primitive plus resource/lifecycle boundary on kernel `6.17.0-1022-azure`, `x86_64`, Landlock ABI 7. A separately pinned AArch64 job targets the standard ephemeral `ubuntu-24.04-arm` runner. | Exact-host x86_64 receipt sets `os_confined=true`; no real analyzer or production admission. The arm64 target cannot inherit that pass, and additional kernel coverage remains required. |
