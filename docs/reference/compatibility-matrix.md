...
| Codex | First-class L1; recorded-scope L2 guidance; experimental CI-3b delivery | An explicit user-level Codex-home TOML kit is validated, safely installed and removed in an isolated `CODEX_HOME`, rejected when malformed, and exercised through the deterministic App Server lifecycle with direct packet equivalence. Its stand-alone project `AGENTS.md` v2 guidance was also exercised by an isolated live Codex CLI smoke. CI-3b adds a separately inspected preview plus explicit `--apply` App Server path, complete initialization, authentication preflight, ephemeral read-only/no-network thread, and authority denial; see the [Phase 1 kit record](../verification/phase-1-codex-connection-kit.md), [L2 guidance record](../verification/phase-2-codex-native-guidance.md), and [CI-3b record](../verification/ci-3b-codex-guided-delivery.md). | L1/L2 remain supported only for their recorded Codex CLI `0.149.0-alpha.4.1` macOS aarch64 evidence. CI-3b is structurally revalidated for App Server `0.150.0-alpha.8`, but its isolated runtime is unauthenticated and declines before packet delivery. It is not L3 admission, successful delivery, or a repeatability claim. |
