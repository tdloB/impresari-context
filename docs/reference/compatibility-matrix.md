...
| Gemini CLI | Generic local MCP | A project `.gemini/settings.json` guide and read-only validator require an absolute local binary, fixed arguments, `trust: false`, and exactly the four released MCP tools. | No installed/authenticated-client, parser, lifecycle, packet, removal, or platform/version evidence yet. |
| GitHub Copilot CLI | Generic local MCP | A project `.mcp.json` guide and read-only validator require the local transport, fixed arguments, and exactly the four released MCP tools. | No installed/authenticated-client, folder-trust, parser, lifecycle, packet, removal, or platform/version evidence yet. |
