...
| Codex | First-class L1; recorded-scope L2 guidance | An explicit user-level Codex-home TOML kit is validated, safely installed and removed in an isolated `CODEX_HOME`, rejected when malformed, and exercised through the deterministic App Server lifecycle with direct packet equivalence. Its stand-alone project `AGENTS.md` v2 guidance was also exercised by an isolated live Codex CLI smoke; see the [Phase 1 kit record](../verification/phase-1-codex-connection-kit.md) and [L2 guidance record](../verification/phase-2-codex-native-guidance.md). | Supported only for Codex CLI `0.149.0-alpha.4.1` on macOS aarch64. It does not claim repository `.codex/config.toml` loading, automatic user-home targeting, deterministic project-instruction discovery, or conversational tool-call repeatability: this App Server version reported no instruction-source record. Revalidate on an upstream client/configuration change. |
