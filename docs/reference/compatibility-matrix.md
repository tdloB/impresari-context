...
| Cursor | Generic local MCP | A non-mutating project/global configuration guide and read-only validator accept Cursor's documented type-less stdio entry while rejecting environment forwarding. Signed-in Cursor Agent CLI `3.17.8` on macOS aarch64 discovered an isolated temporary project entry without enabling it. A malformed isolated entry was not loaded and left its source fixture unchanged; see the [Phase 1 kit record](../verification/phase-1-cursor-connection-kit.md). | Not first-class: user-approved MCP use, tool lifecycle and packet evidence, broader platform/version coverage, and user-owned project-entry removal evidence remain. |
