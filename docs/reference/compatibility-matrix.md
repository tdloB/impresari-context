...
| ADR-0074 macOS IAR-1B candidate | Selected partial feasibility; IAR-1A remains supported posture | Ad hoc signed App Sandbox/private-XPC prototype plus synthetic supervisor proved bounded IPC, selected access denials, CPU termination, address-space-growth denial, `fork`/`posix_spawn` denial, exact-target timeout termination, crash/relaunch, and source-byte cleanup on macOS `26.5.1` arm64 | No real analyzer backend or production admission. Device denial, frozen production profiles, Developer ID/notarization, ADR-0076 cask lifecycle, full Tier A, and multi-host evidence remain open. |
