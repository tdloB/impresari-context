...
| ADR-0074 isolated analyzer runner | IAR-0 protocol model | Closed v1 schemas, fixed synthetic profile, pure in-memory no-op/fault worker | No subprocess or filesystem runner, OS confinement, ClamAV, YARA, reputation provider, analyzer discovery, signature lifecycle, or real analyzer execution exists. |
