...
| ADR-0089/0095/0096 YARA adapter and supply chain | Contract-only synthetic/metadata checkpoint | Digest-bound synthetic-only adapter; exact YARA v4.5.8 tag commit and license metadata; 30-day source-selection expiry; separate future executable/ruleset identities; mandatory build, SBOM, provenance, signing, review, expiry, rollback, and revocation gates | No source archive, YARA binary, ruleset, parser, process launch, repository analyzer input, network, credential access, live result, detection claim, IAR-2, production admission, safety verdict, or added authority. |
