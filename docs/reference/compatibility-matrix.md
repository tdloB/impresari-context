...
| ADR-0089/0095 YARA adapter | Contract-only synthetic checkpoint | Digest-bound synthetic-only profile; closed production-shaped result and normalization receipt; complete artifact accounting; canonical bounded rule observations; exact artifact and byte-range bindings; domain-separated match identities; original-synthetic provenance | No YARA binary, ruleset, parser, process launch, repository analyzer input, network, credential access, live result, detection claim, IAR-2, production admission, safety verdict, or added authority. |
