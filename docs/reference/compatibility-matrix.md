...
| ADR-0087 macOS local-VM candidate | Partial synthetic supervisor lifecycle passed; IAR-1A remains supported posture | On macOS `26.5.1` arm64, fresh guests proved exact read-only input, hard scratch capacity, absent guest networking, cross-job cleanliness, reproducible exact guest identity, tampered-guest rejection, bounded output-flood and malformed-result rejection, whole-VM timeout and forked-descendant stop, early-exit cleanup, controller cancellation, and post-fault recovery. The Rust supervisor additionally passed pre-launch controller-digest verification, exact external cancellation, forced controller kill/reap, exact stale-job removal, and recovery after either action through its single audited launch site. All receipts retain `vm_confined=false`. | One-host partial synthetic evidence only. Atomic sealed-controller identity, host interruption, guest memory/CPU, full host-canary corpus, supply-chain policy, multi-host, Developer ID/notarization, one-cask lifecycle, independent review, production admission, and real analyzers remain unverified. |
