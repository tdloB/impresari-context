...
| ADR-0074 Linux IAR-1B candidate | Native x86_64 and arm64 exact-host composites passed; broader Linux pending | PR 131 job `99197119262` passed x86_64 and PR 132 job `99198568879` independently passed arm64 on kernel `6.17.0-1022-azure`, Landlock ABI 7, each with one atomically placed source-free worker under the full primitive and resource/lifecycle boundary. | Both exact-host receipts set `os_confined=true`; no real analyzer or production admission. A materially independent kernel target and frozen production support scope remain required. |
