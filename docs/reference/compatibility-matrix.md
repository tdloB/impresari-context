...
| ADR-0087 macOS local-VM candidate | Synthetic feasibility in progress; IAR-1A remains supported posture | On macOS `26.5.1` arm64, two fresh local Linux VM jobs proved exact read-only synthetic input, a hard 1 MiB scratch capacity, absent non-loopback guest networking, no cross-job marker retention, VM stop, and exact per-job removal. The profile, raw kernel, initramfs, and source-free receipts are exact and retain `vm_confined=false`. | One-host storage/cross-job checkpoint only. Escape, descendant/resource, lifecycle, malformed-result, supply-chain, multi-host, Developer ID/notarization, one-cask lifecycle, independent review, production admission, and real analyzers remain unverified. |
