...
| ADR-0087 macOS local-VM candidate | Partial synthetic interruption checkpoint passed; IAR-1A remains supported posture | On macOS `26.5.1` arm64, fresh guests proved exact read-only input, hard scratch capacity, absent guest networking, cross-job cleanliness, reproducible exact guest identity, bounded malformed/output/fault handling, whole-VM descendant stop, Rust-supervised lifecycle recovery, cgroup v2 resource enforcement, and six host-only canary classes. The controller now installs a macOS will-sleep observer; its shared stop/cleanup/recovery handler passed using a source-free synthetic trigger through the single audited launch site. All receipts retain `vm_confined=false`. | One-host partial synthetic evidence only. The automated interruption receipt requires `real_host_sleep_observed=false`; genuine sleep/wake, reboot, and power-loss recovery remain open. Atomic sealed-controller identity, sealed guest supply-chain policy, multi-host, Developer ID/notarization, one-cask lifecycle, independent review, production admission, and real analyzers remain unverified. |
