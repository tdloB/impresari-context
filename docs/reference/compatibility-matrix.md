...
| ADR-0087 macOS local-VM candidate | Partial synthetic interruption and fail-closed guest-review checkpoints passed; IAR-1A remains supported posture | On macOS `26.5.1` arm64, fresh guests proved exact read-only input, hard scratch capacity, absent guest networking, cross-job cleanliness, reproducible exact guest identity, bounded malformed/output/fault handling, whole-VM descendant stop, Rust-supervised lifecycle recovery, cgroup v2 resource enforcement, and six host-only canary classes. The controller's source-free will-sleep path passed. Alpine publisher authentication passed, and the exact stale `6.18.35-0-virt` guest was denied after comparison with current `6.18.48-r0`. | One-host partial synthetic evidence only. The guest must be replaced; provider data did not establish complete advisory coverage, so `vulnerability_assessment_complete=false`. Genuine sleep/wake, reboot, power-loss recovery, sealed release metadata, multi-host, Developer ID/notarization, one-cask lifecycle, independent review, production admission, and real analyzers remain unverified. |
