...
| ADR-0074 Linux IAR-1B candidate | Unsupported on the observed hosted Ubuntu runner; unadmitted | Kernel `6.17.0-1022-azure`, `x86_64`, Landlock ABI 7 passed the source-free `no_new_privs`, read-only-input/zero-write Landlock, default-deny seccomp, descriptor, network, and descendant primitive suite | The hosted job cgroup was not delegated, so the receipt kept `os_confined=false`. No real analyzer or production admission. Delegated cgroup resource, kill/empty-state, timeout, crash, cleanup, cross-job, multi-kernel, and `aarch64` filter evidence remain required. |
