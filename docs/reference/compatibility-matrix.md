...
| Codex | First-class (recorded scope) | An explicit user-level Codex-home TOML kit is validated, safely installed and removed in an isolated `CODEX_HOME`, rejected when malformed, and exercised through the deterministic App Server lifecycle with direct packet equivalence; see the [Phase 1 kit record](../verification/phase-1-codex-connection-kit.md). | Supported only for Codex CLI `0.149.0-alpha.4.1` on macOS aarch64. It does not claim repository `.codex/config.toml` loading, automatic user-home targeting, or conversational tool-call repeatability. Revalidate on an upstream client/configuration change. |
| Claude Code | First-class (recorded scope) | An explicit local-scope command and read-only JSON validator preserve the fixed local-stdio contract. Claude Code CLI `2.1.241` on macOS aarch64 rejected malformed strict temporary configuration, completed an isolated temporary-config lifecycle with direct packet equivalence, and completed native `claude mcp add/get/remove --scope local` against a disposable Claude home. See the [Phase 1 kit record](../verification/phase-1-claude-code-connection-kit.md). | Supported only for Claude Code CLI `2.1.241` on macOS aarch64. The model-directed lifecycle is live smoke evidence, not deterministic prompt-repeatability. The native rehearsal never writes a default user home and removes only the named entry in its explicit temporary home. Revalidate on an upstream client/configuration change. |
| Cursor | First-class (recorded scope) | An explicit project `.cursor/mcp.json` kit and validator preserve the fixed local-stdio contract. Cursor Agent CLI `3.17.8` (`2026.08.11-e8db854`) on macOS aarch64 rejected malformed configuration, completed native enable/list-tools/disable, and completed a guarded Agent-mode four-tool lifecycle with direct packet equivalence in a disposable project. See the [Phase 1 kit record](../verification/phase-1-cursor-connection-kit.md). | Supported only for the recorded Cursor CLI/macOS aarch64 scope. Ask mode blocks dynamic MCP calls; the recorded Agent-mode smoke used a test-only project permission file allowing only the four named Impresari MCP tools and denying shell/file/web actions. Conversational tool choice remains non-deterministic. Revalidate on an upstream client/configuration/approval/stream change. |
...
| GitHub Copilot CLI | Generic local MCP | A project `.mcp.json` guide and read-only validator require the local transport, fixed arguments, and the minimal four-tool session/packet allowlist. Copilot CLI `1.0.80` on macOS aarch64 completed an isolated temporary-config lifecycle with malformed-configuration rejection, direct packet equivalence, exact removal, and source immutability; see the [Phase 2 kit record](../verification/phase-2-copilot-cli-connection-kit.md). | Not first-class: user-reviewed project-local install, folder trust, and single-entry removal remain. Its conversational tool choice is live-smoke evidence, not deterministic conformance. |
