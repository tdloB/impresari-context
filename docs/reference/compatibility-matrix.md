...
| ADR-0074 Linux IAR-1B candidate | Composite checkpoint implemented; hosted evidence pending | Kernel `6.17.0-1022-azure`, `x86_64`, Landlock ABI 7 passed the primitive suite. PR 130 job `99194709845` separately passed the delegated resource/lifecycle component. The next check applies both to one atomically placed source-free worker inside the same one transient delegated service. | No real analyzer or production admission. Only a new composite receipt may set exact-host `os_confined=true`; multi-kernel and independently admitted architecture evidence remain required. |
