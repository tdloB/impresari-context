...
| ADR-0074 Linux IAR-1B candidate | Native x86_64 and arm64 exact-host composites passed; held-out kernel matrix pending | PR 131 job `99197119262` passed x86_64 and PR 132 job `99198568879` independently passed arm64 on kernel `6.17.0-1022-azure`, Landlock ABI 7. Native Ubuntu 22.04 and 26.04 jobs now target separate 6.8 and 7.0 kernel lines. | Exact-host receipts set `os_confined=true`; no real analyzer or production admission. Each new kernel must pass independently before the production support scope is frozen. |
