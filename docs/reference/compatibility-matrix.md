...
| ADR-0088/0092 Windows native candidate | Exact contract frozen; native no-worker preflight pending | The target profile fixes a fresh zero-capability LPAC/AppContainer identity, Job Object and mitigation posture, bounded read-only staging/results, resources, and cleanup. A dedicated Windows 2025 x86-64 hosted job is required to prove exact build/NTFS/API availability, empty Job Object set/query, and AppContainer create/derive/delete lifecycle. | No worker has run inside AppContainer. Network/path/resource/descendant denial, full cleanup, multi-host evidence, signing, production admission, Windows IAR-1B, and real analyzers remain unverified. |
