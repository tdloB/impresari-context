...
| ADR-0094 Windows BaseContainer routing candidate | Read-only capability contract implemented; hosted observation pending | A dedicated Windows 11 arm64 job may inspect only the exact Windows product/build/filesystem and trusted System32 `processmodel.dll` exports. It cannot launch a process, invoke the sandbox API, create a profile, mutate ACLs/features, elevate, install a service, or add authority. | Minimum build is `26600`; unsupported builds or missing exports fail closed. Even a ready result is capability evidence only, not Windows IAR-1B, production support, or analyzer admission. |
