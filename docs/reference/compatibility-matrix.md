...
| ADR-0074 Linux IAR-1B candidate | Primitive and delegated-resource components measured separately; overall unadmitted | Kernel `6.17.0-1022-azure`, `x86_64`, Landlock ABI 7 passed the primitive suite. In PR 130 job `99194709845`, a separate ephemeral systemd-delegated subtree passed atomic placement, CPU, memory, pids, exact kill/empty-state, bounded output, timeout, crash/relaunch, cleanup, and cross-job checks. | Component receipts keep `os_confined=false`; no real analyzer or production admission. One source-free worker must reproduce both boundaries together, followed by multi-kernel and independently admitted architecture evidence. |
