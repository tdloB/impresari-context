...
| ADR-0087 macOS local-VM candidate | Partial synthetic resource/canary checkpoint passed; IAR-1A remains supported posture | On macOS `26.5.1` arm64, fresh guests proved exact read-only input, hard scratch capacity, absent guest networking, cross-job cleanliness, reproducible exact guest identity, bounded malformed/output/fault handling, whole-VM descendant stop, and Rust-supervised external lifecycle recovery. A separate exact guest additionally passed cgroup v2 memory OOM containment, CPU throttling, process accounting/removal, exact-device observation, and six host-only path/content/process canary classes through the single audited launch site. All receipts retain `vm_confined=false`. | One-host partial synthetic evidence only. Atomic sealed-controller identity, host sleep/reboot/power-loss recovery, sealed guest supply-chain policy, multi-host, Developer ID/notarization, one-cask lifecycle, independent review, production admission, and real analyzers remain unverified. |
