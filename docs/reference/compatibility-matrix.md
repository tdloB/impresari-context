...
| ADR-0074 macOS IAR-1B candidate | Selected partial feasibility; IAR-1A remains supported posture | Ad hoc signed App Sandbox/private-XPC prototype plus synthetic supervisor proved bounded IPC; filesystem, credential, pseudo-terminal device, process, and network denials; effective frozen resource limits; closed source-free launch preparation; exact-target timeout; crash/relaunch; and source-byte cleanup on macOS `26.5.1` arm64 | No real analyzer backend or production admission. Developer ID/notarization, ADR-0076 cask lifecycle, clean-machine Gatekeeper, full Tier A, OS-managed container removal, and multi-host evidence remain open. |
