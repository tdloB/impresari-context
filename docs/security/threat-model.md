...
| SEC-T-052 | Linux primitive, cgroup, composition, or support-scope overclaim | Kernel API presence, a partial Landlock/seccomp probe, controller inventory, a writable cgroup path, separate component passes, or a finite exact-host corpus is presented as complete per-job OS confinement or broad Linux support | Closed profiles/receipts; `no_new_privs`; exact read-only staged input; architecture-pinned default-deny seccomp; descriptor closure; zero writable path-backed filesystem; one transient delegated subtree; unprivileged supervisor; profile limits applied before atomic `CLONE_INTO_CGROUP`; primitive denials reproduced inside that exact worker; exact kill and empty-state verification; production admission remains false | Native synthetic primitive check; CI-only delegated component check; single-service composite check; x86_64/arm64 plus held-out 6.8/7.0 kernel corpus; schema overclaim fixtures; frozen profiles/provenance | CI sudo is limited to transient-service creation. Exact-host candidate evidence is not a broad Linux or production-support promise, and no candidate evidence authorizes real analyzer execution |
