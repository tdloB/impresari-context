...
| SEC-T-055 | Windows native capability or cleanup overclaim | API presence, an empty Job Object, a transient AppContainer profile, or one hosted build is presented as a confined worker or broad Windows support | Digest-bound target profile; fresh hosted-VM guard; exact build/architecture/NTFS receipt; zero-capability profile; created/derived SID equality; mandatory deletion; empty Job Object set/query; breakaway rejection; all worker, denial, confinement, production, analyzer, and authority fields fixed false | ADR-0092 profile/receipt schemas, exact checker, invalid overclaim fixture, and dedicated Windows native preflight | Preflight does not launch a worker or measure LPAC, ACL, handle, mitigation, network, resource, descendant, crash, reboot, or cross-job behavior; IAR-1B remains open |
| SEC-T-056 | Windows worker runs before confinement, inherits ambient authority, writes AppContainer profile storage, or leaves cross-job residue | A child executes before Job assignment; extra handles/environment/capabilities cross the boundary; default per-profile storage violates zero-write scope; cleanup or identity reuse leaks state | Suspended creation; zero-capability LPAC; exact handle list; fixed mitigations and child restriction; pre-resume Job assignment; exact ACL staging; profile-storage write denial; filtered non-caller environment; bounded pipes; one cleanup guard; fresh identity and second-job canary | ADR-0093 closed matrix profile/receipt schemas, invalid overclaim fixture, source-free checker, and PR 183 hosted launch attempt | Windows Server 2025 build 26100 denied the first suspended LPAC worker before execution because the unmodified host lacks required least-privilege launch access. The receipt is `unsupported_host`; effective denials, fault containment, independent hosts, production, and real analyzers remain unverified |
| SEC-T-057 | Windows host preparation or experimental BaseContainer availability is mistaken for sandbox admission | An installer elevates to rewrite host ACLs, a missing or drifted experimental API is silently bypassed, or API presence is reported as confinement | No automatic elevation, service, feature enablement, or host ACL mutation; exact workstation/build/architecture/NTFS target; trusted System32 module/export inspection only; deterministic unsupported states; every launch and authority claim fixed false | ADR-0094 profile and receipt schemas, source-free contract checker, provenance-bound fixtures, and dedicated Windows 11 ARM no-worker CI | Capability presence can authorize only a later synthetic rehearsal. It does not prove worker launch, denial behavior, Job Object composition, cleanup, IAR-1B, production, or real-analyzer safety |
| SEC-T-058 | YARA adapter contract or match laundering | A path, source byte, raw output, unmatched artifact, out-of-range offset, noncanonical rule observation, forged digest, synthetic fixture, or zero-match result is presented as live analysis or a safety verdict | Synthetic-only closed profile/result/receipt schemas; fixed byte/count limits; complete artifact accounting; canonical ordering; domain-separated match identities; exact artifact/range binding; provenance hashes; no YARA reference in production Rust; execution, confinement, production, IAR-2, safety, host authority, and added authority fixed false | ADR-0095 schema-valid fixtures, path and overclaim rejection fixtures, deterministic offline normalization checker, profile/fixture digest checks, and production-source scan | The contract proves only the shape and deterministic handling of original-synthetic data. It does not validate a YARA executable, rules, raw-output parser, isolation backend, live result, detection quality, or absence of malware |
| SEC-T-059 | YARA source, executable, or ruleset substitution | A moved tag, mutable latest URL, package-manager binary, absent or stale evidence, unreviewed repository rule, rollback, or revoked artifact is presented as an admitted analyzer component | Exact official tag commit/license metadata; source/executable/ruleset identity separation; no accepted upstream asset; 30-day source-selection expiry; mandatory per-target build, archive SHA-256, dependency closure, SBOM, provenance, reproducibility, signature, vulnerability/license review, ruleset review, expiry, rollback, and revocation identities; repository rules/includes/external paths/custom modules/network/update credentials denied | ADR-0096 closed profile/receipt schemas; exact digest/provenance checker; tag-movement and overclaim fixtures; deterministic revocation/expiry/movement/missing-artifact state vectors; rule-artifact absence scan | Public metadata selection does not authenticate a future downloaded archive or build. No executable or ruleset exists in the checkpoint, and later artifact creation, platform confinement, hosted evidence, and independent review remain required |
| SEC-T-060 | YARA-X build, feature, compatibility, or isolation laundering | A mutable archive, broad default-module build, reachable vulnerable dependency, unreviewed rule, repository scan input, raw match bytes, partial fixture pass, stale host, leaked artifact, or synthetic result is presented as production analyzer admission | Immutable commit archive and exact size/digest; content-addressed three-file patch; frozen patched manifests/lock; exact Rust target and command; selected-feature graph excludes modules/RSA/X.509/spin/Wasmtime-WASI/cap-std; enumerated advisory dispositions; original-synthetic rule digest and narrow policy; generated scan inputs; exact NDJSON assertions; fresh atomic cgroups; read-only Landlock; architecture-pinned seccomp; bounded CPU/memory/tasks/time/output; mandatory cleanup; every production/IAR-2/detection/safety field false | ADR-0099 profile/receipt schemas, patch/rule/fixture provenance checks, archive validator, rule-policy rejection corpus, static contract checker, Linux launcher compilation, and maintainer-triggered hosted synthetic rehearsal | The runner image is mutable; artifacts are unsigned and ephemeral; bincode is unmaintained; full lockfile advisories require feature-reachability dispositions. Compatibility evidence does not establish detection quality, repository safety, reproducibility, production support, or IAR-2 |
| SEC-T-061 | Hostile or malformed YARA-X NDJSON laundering | Vendor-shaped output injects unknown or duplicate fields, paths, matched bytes, overflowed ranges, partial results, noncanonical identities, or false execution/safety claims into normalized evidence | Pure one-record parser; strict LF/UTF-8 framing; unknown/duplicate-field denial; exact staged-path equality with path omission; exact zero-byte marker grammar; checked ranges and fixed ceilings; canonical all-or-nothing output; source-free errors; every execution/confinement/production/IAR-2/safety field false | ADR-0100 profile/control/result schemas, exact profile digest, provenance-bound positive/negative corpus, deterministic result vector, dependency and capability guard, unit and mutation tests | Pure parsing does not authenticate that an analyzer ran, prove OS confinement, admit an artifact/ruleset, measure detection quality, or authorize repository-derived analyzer input |
| SEC-T-062 | Short-lived YARA-X candidate retention or custody laundering | An unexpected archive member, link, mutable image, changed dependency, leaked source/rule, extended lifetime, second upload, artifact substitution, or unadmitted executable is presented as a signed, published, safe, private-to-maintainers, or production analyzer | Exact Linux/amd64 image manifest and OCI-index identities; canonical source/patch/lock/toolchain/feature/build command; network-disabled build container; closed twelve-member archive; 256 MiB cap; exact manifest/checksums/SPDX/provenance/dispositions; one commit-pinned upload; fixed authenticated-reader seven-day retention; explicit public-repository/non-maintainer-only disclosure; separate same-run non-executing TarReader verification; all signing, execution, admission, detection, safety, and IAR-2 claims fixed false | ADR-0104 profile/receipt schema, valid and overclaim fixtures, profile sidecar, static workflow/runner/packager/verifier checker, and manually dispatched same-run verification | GitHub Actions remains the temporary custodian; any signed-in repository reader may download the artifact, and repository/action/platform compromise remains possible. The artifact is unsigned, unattested, temporary, unreviewed by a human, and never production-admitted |
| SEC-T-063 | Synthetic macOS app-tree or distribution laundering | A traversing path, symlink, special file, executable marker, unexpected member, changed seal, nondeterministic tree, retained temporary bundle, or synthetic assembly is presented as a release, signed app, working cask, VM sandbox, or IAR-1B admission | Digest-bound contract/spec/profile; closed 13-entry tree; fresh non-symlinked temporary roots; exact target modes and `0700` parent on macOS/POSIX; non-executable labelled placeholders; exact synthetic `Info.plist`; exact metadata-seal copy; two-run canonical tree equality; mandatory cleanup; every release, install, signing, notarization, cask, VM, analyzer, production, and IAR-1B claim fixed false | ADR-0108 schemas, valid and overclaim fixtures, provenance hashes, portable two-run assembler/checker, target-mode/byte/digest checks, and repository policy integration | Windows CI proves only portable path, kind, byte, digest, determinism, and cleanup properties because Windows mode bits are not macOS ACL or executable-bit evidence. Filesystem composition alone does not establish valid release binaries, code signing, notarization, install behavior, runtime confinement, VM operation, or analyzer safety |
| SEC-T-064 | macOS candidate source, build, artifact, guest, or release-identity laundering | A documentation baseline, partial source inventory, changed command, missing bundle role, unrecorded Apple toolchain, unreviewed binary, synthetic guest, absent SBOM/vulnerability record, or mixed rollback is presented as a real release identity | Exact baseline revision and direct-input canonical digest; closed ADR-0107 role mapping; locked Rust target/commands; exact ADR-0091 guest binding; frozen future candidate schema requiring full source revision/archive, build-host/toolchain, per-artifact, SBOM/license/vulnerability/reproducibility, compound identity, and rollback evidence; every current candidate, signing, distribution, runtime, production, and IAR-1B claim false | ADR-0109 contract/profile/candidate/receipt schemas, digest-bound offline checker, valid receipt/profile fixtures, provenance inventory, and invalid candidate-materialization overclaim | The direct input set is build-control metadata, not the full source closure. No candidate build, Apple environment, artifact identity, product SBOM, vulnerability disposition, reproducibility result, signature, notarization, install, VM, or analyzer evidence exists yet |
| SEC-T-065 | Ephemeral macOS product build, signature, reproducibility, or cleanup laundering | A dirty source tree, mutable epoch, online dependency resolution, missing unit, linker ad-hoc signature, one-host equality, retained binary/log, or product-only record is presented as a Developer ID signed, reproducible, complete, or production release | Exact clean source revision/archive and commit epoch; two distinct private roots; locked offline non-incremental Cargo; private Swift caches; exact host/toolchain/artifact/log/Mach-O/code-directory/dynamic-library identities; explicit linker-ad-hoc versus Developer ID distinction; bound SPDX/license/advisory/reproducibility dispositions; mandatory deletion; guest, app, distribution, VM, analyzer, production, and IAR-1B claims false | ADR-0110 profile/record/receipt schemas, digest-bound metadata checker, approved original-metadata fixtures, invalid release overclaim, and absence check for disposable roots after the live rehearsal | Same-host same-toolchain byte equality does not establish cross-run, cross-host, or production reproducibility. The frozen SBOM describes the source dependency closure, not a signed release archive. No guest, app, Apple signature, notarization, install, cask, VM, analyzer, or production evidence exists |
| SEC-T-066 | macOS synthetic guest payload, upstream, build-input, or materialization laundering | An open guest directory, extra test intermediate, changed controller name, substituted APK/key/build input, executable mode, retained temporary output, or source-free contract is presented as a materialized guest or release | Closed sorted two-file mode-0644 runtime projection; exact manifest/seal/controller bindings; explicit four-component exclusion; exact APK bytes/hash/key/signature/datahash/package identity; exact extraction, project source, Zig target/options, canonical initramfs recipe, private-root and cleanup requirements; all materialization, release, distribution, VM, analyzer, production, and IAR-1B claims false | ADR-0111 contract/profile/receipt schemas, digest-bound offline checker, approved original-metadata fixtures, invalid materialization overclaim, registry and conformance coverage | The contract does not download, build, retain, launch, sign, notarize, install, publish, or prove runtime confinement. The current payload remains synthetic-only and the guest vulnerability review remains time-bound and incomplete for production |
| SEC-T-067 | macOS synthetic guest rehearsal, executable custody, or cleanup laundering | A redirected or substituted APK, unsafe archive member, changed toolchain, unexpected output, retained temporary path, or unexecuted synthetic build is presented as a shipped, safe, confined, or production guest | Exact HTTPS endpoint with zero redirects; bytes/digest and APKv2 signature/datahash/package/commit verification; two bounded regular-file selections; exact project input digests and Zig target; two exact mode-0644 output identities; no destination argument; fresh mode-0700 root; bounded child output; complete success/failure cleanup before metadata | ADR-0112 profile/record/receipt schemas, digest-bound materializer and offline checker, original-project metadata fixtures, invalid VM-launch overclaim, and one successful local macOS arm64 materialize-and-delete rehearsal | The synthetic guest was built but never executed, retained, assembled, signed, notarized, installed, published, or tested as a VM boundary. Network availability and upstream custody remain external dependencies; production and macOS IAR-1B remain false |
| SEC-T-068 | macOS release-candidate composition or co-custody laundering | Separately built and deleted product and guest identities, synthetic tree metadata, or a prospective digest is presented as one assembled candidate, verified mode set, bound release identity, or distributable app | Exact digest bindings for five prior evidence sources and two contracts; closed sorted eight-file projection; explicit required versus observed modes; domain-separated prospective identity; every co-materialization and later gate explicit false; separate readiness schema rather than the materialized-candidate schema | ADR-0113 profile/composition/receipt schemas, digest-bound offline checker, original-project metadata fixtures, and invalid candidate-materialized overclaim | No complete candidate existed. Components were never held in one private root, candidate modes and full tree were not verified, and no app, Apple identity, distribution, runtime, production, or IAR-1B evidence was created |
| SEC-T-069 | macOS unsigned candidate custody, identity, cleanup, or distribution laundering | A changed source or public package, partial build, escaped executable, extra tree member, wrong mode, ad-hoc signature, retained app, or ephemeral candidate is presented as Developer ID signed, notarized, installed, sandboxed, or production-ready | Exact Git archive and APKv2 authentication; locked offline product builds; private Cargo/Swift/Zig roots; one mode-0700 custody boundary; closed eight-file app inventory; exact modes, byte identities, contract and material digests; inspection without execution; mandatory whole-root deletion; all Apple, distribution, runtime, production, and IAR-1B gates false | ADR-0114 profile/candidate/rehearsal/receipt schemas, digest-bound static checker, live macOS arm64 build-assemble-delete rehearsal, original metadata fixtures, and invalid signing overclaim | The unsigned app existed only ephemerally and was deleted. Linker ad-hoc identity is not Developer ID signing. No archive, cask, install, notarization, VM, analyzer, production, or macOS IAR-1B evidence exists |
| SEC-T-070 | Apple signing credential exposure or notarization laundering | A credential value enters source, arguments, environment, logs, fixtures, or artifacts; nested code is signed ambiguously; an accepted submission hides errors or warnings; or a signed/notarized synthetic candidate is presented as installed, published, production, or IAR-1B | Opaque existing-Keychain references only; founder-controlled manual boundary; explicit inside-out signing with hardened runtime and secure timestamp; one frozen controller entitlement; strict recursive verification; required accepted log inspection and warning disposition; staple, validate, Gatekeeper assess, final archive recreation, private-root deletion, and source-free metadata-only receipt | ADR-0115 profile/contract/receipt schemas, digest-bound checker, original metadata fixtures, and invalid credential/signing/notarization overclaim | The current checkpoint is offline preparation only. It accesses no identity, contacts no service, creates no artifact, and cannot establish cask lifecycle, clean-machine behavior, production, or macOS IAR-1B |
| SEC-T-071 | Product read-accounting omission or laundering | Startup discovery, repeated reads, rejected bytes, skipped objects, counter overflow, caller values, packet contents, or stale source identity are omitted or transformed into a false efficiency claim | Always-on process-local observer at `AuthorizedWorkspace::read_exact`; exact byte and repeat counters; checked arithmetic; portable source fingerprint from admitted bytes; incomplete on overflow, synchronization failure, incompatible path, partial discovery, or any skip; closed source-free MCP projection | Workspace counter/fingerprint/negative tests, engine completeness tests, MCP closed-shape tests, and independent provider-free cross-repository compatibility | Process-lifecycle counters are not per-call deltas and do not prove usefulness. The evaluator must use a fresh one-build process and separately measure adapter and agent work |
| SEC-T-072 | Task-prose query suppression or syntax smuggling | Ordinary words force an explicit anchor out of lexical candidates, or task text is treated as FTS, glob, regex, shell, path, or graph authority | Closed local task-signal scanner; exact/path/code/lexical classes; frozen stop words; eight-step ceiling; explicit original-query omission; existing capability-relative retrieval; planner order carried into bounded packaging | Signal vectors, noisy exact-anchor recovery, long/control/shell/traversal/FTS negative tests, packet-priority tests, and provider-free evaluator stability gate | Version 1 recognizes only deterministic ASCII signals and does not provide semantic intent understanding or automatic graph traversal |
| SEC-T-073 | Structural-seed ambiguity or task-to-graph authority escalation | Generic prose, duplicate symbol names, hostile node-looking text, or evaluator-supplied identifiers select a broad or wrong traversal | Only admitted exact path/code signals; exact case-sensitive graph matching; one-seed ceiling; path-scoped symbol preference; ambiguity produces explicit no-seed fallback; existing snapshot validation and traversal limits | Unique/scoped/ambiguous/hostile seed vectors, stale-graph tests, depth/node/edge limit tests, packet-priority tests, and provider-free utility comparison | Version 1 does not build a graph in MCP, use lexical terms as seeds, traverse reverse edges, or provide semantic disambiguation |
| SEC-T-074 | Structural utility laundering | A working graph query is presented as useful, efficient, or correctness-improving while exact anchors are displaced, graph-build reads are omitted, packet growth is hidden, or provider conclusions are inferred from local fixtures | Frozen same-source fresh-engine comparison; ordered anchor-prefix requirement; exact structural novelty; complete read telemetry; packet-growth ceiling; source immutability; explicit exclusion of graph construction and model claims | Six-fixture TypeScript/Rust/Ruby provider-free gate, deterministic rerun, read/repeat/byte deltas, mutation check, and hosted matrix | The gate establishes bounded mechanical utility only. External graph lifecycle must include cold graph-build cost, and provider/SWE-bench conclusions require separately authorized controlled studies |
| SEC-T-075 | Structural lifecycle, worker-cache, or treatment-contract laundering | A caller-selected graph/node, partial startup tuple, stale parser result from another worker binary, hidden graph-build work, silent fallback, or treatment-only tool is presented as product-owned comparable structural context | All-or-none trusted startup tuple; absolute non-overlapping non-symlink paths; exact worker digest in control-process cache lineage; pre-readiness graph preparation; same MCP tools/request grammar; core-owned seed selection; cumulative read telemetry and closed lifecycle receipt; fail-closed structural intent | Startup parser and overlap negatives, worker digest/cache-lineage tests, cold/warm process tests, MCP equivalence, source mutation checks, and hosted macOS/Linux/Windows matrix | Static structural delivery can increase packet size and still does not prove token, latency, cost, or correctness improvement. Long-lived refresh and progressive disclosure remain separate decisions |
| SEC-T-076 | Progressive-handle authority, budget, or exactness laundering | A forged, foreign, stale, closed-session, or replayed map/item handle selects arbitrary graph/source state; repeated lookups exceed limits; compact labels are presented as exact evidence; or partial expansion is hidden as success | Domain-separated content identities plus same-process consumer/session/workspace/snapshot/graph/content binding; handles select only stored validated descriptors; exact source only through reauthorized evidence expansion; monotonic cumulative operation/item/source-byte/response-byte/read/repeat/time ledger with pre-reservation; explicit partial/exhausted/unavailable receipts; equal tools across modes | Closed schemas, cross-session/workspace/process and mutation negatives, first-over-limit property tests, eager/full-expansion byte equivalence, source immutability, tool parity, deterministic identity, and provider-free ordinary/eager/progressive mechanics gate | Compact labels still consume context and may induce extra tool calls. The provider-free gate cannot establish model choices, token/cost/latency improvement, or correctness; durable memory and generated summaries remain excluded |
| SEC-T-077 | Historical/current release-evidence substitution | A historical metadata-integrity pass is presented as proof that current source matches an old candidate, or current source hashes silently replace historical evidence | Immutable historical records; distinct historical and current profiles/schemas/receipts/jobs; historical result fixed to `historical_not_current` with current/release claims false; newly versioned current lineage bound to an exact source archive/revision; release workflows accept only current-candidate success | Historical mutation and current-overclaim fixtures, one-byte current-source mutation, lineage substitution tests, source-free cross-platform determinism, and static release-workflow dependency checks | Recorded historical metadata does not reproduce old source bytes during ordinary CI. A separate local archive is required for deep historical byte audit, and a new candidate remains necessary before release |
...
| SEC-REQ-022 | Measure context-product repository reads at the capability-relative byte boundary, bind complete telemetry to exact isolated source identity, and reject any performance admission when accounting is partial |
| SEC-REQ-023 | Decompose bounded task prose before retrieval, preserve explicit anchors under noise and packet limits, and never interpret task syntax as execution, path, query-language, or graph authority |
| SEC-REQ-024 | Select at most one structural seed from exact admitted path/code signals, reject ambiguity without aborting ordinary retrieval, and retain all existing snapshot, traversal, source-verification, and packet bounds |
| SEC-REQ-025 | Require a frozen provider-free comparison to preserve exact anchors, expose structural novelty and all added product reads/bytes, cap packet growth, prove immutability/determinism, and prohibit model claims before external structural delivery |
| SEC-REQ-026 | Bind optional structural MCP preparation to an all-or-none trusted startup tuple, include the exact worker digest in parser-cache identity, keep baseline/treatment tools and request grammar equal, expose complete cold-lifecycle accounting, and fail closed without adapter-selected graph authority |
