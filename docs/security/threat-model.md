...
| SEC-T-047 | Coverage laundering or assessment substitution | Forged inventory fields, altered coverage states, zero findings, or mismatched evidence attempt to produce a complete or permissive assessment | Recompute inventory and coverage identities; exact snapshot/artifact/finding/evidence binding; mandatory unavailable coverage forces partial; all authority and safety fields false | HRA-3 deterministic, schema, forgery, laundering, missing-analysis, and output-bound tests | External analyzer results remain unsupported until their separate envelope boundary is implemented |
...
| HRA-3 coverage and assessment conformance | Deterministic capability grouping, exact artifact binding, unavailable mandatory analysis visibility, forged-state rejection, immutable identities, schema-valid output, and constant no-safety/no-authority fields |
