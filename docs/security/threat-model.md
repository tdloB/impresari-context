...
| SEC-T-063 | Synthetic macOS app-tree or distribution laundering | A traversing path, symlink, special file, executable marker, unexpected member, changed seal, nondeterministic tree, retained temporary bundle, or synthetic assembly is presented as a release, signed app, working cask, VM sandbox, or IAR-1B admission | Digest-bound contract/spec/profile; closed 13-entry tree; fresh non-symlinked temporary roots; exact `0700` target policy on macOS/POSIX; non-executable labelled placeholders; exact synthetic `Info.plist`; exact metadata-seal copy; two-run canonical tree equality; mandatory cleanup; every release, install, signing, notarization, cask, VM, analyzer, production, and IAR-1B claim fixed false | ADR-0108 schemas, valid and overclaim fixtures, provenance hashes, portable two-run assembler/checker, exact mode/byte/digest checks, and repository policy integration | Windows CI proves only portable structure, determinism, and cleanup because Windows mode bits are not macOS ACL evidence. Filesystem composition alone does not establish valid release binaries, code signing, notarization, install behavior, runtime confinement, VM operation, or analyzer safety |
