...
| SEC-T-052 | Linux primitive or cgroup overclaim | Kernel API presence, a partial Landlock/seccomp probe, controller inventory, or a writable cgroup path is presented as complete per-job OS confinement | Closed profile and receipt; `no_new_privs`; exact read-only staged input; architecture-pinned default-deny seccomp; unrelated-descriptor closure; zero writable path-backed filesystem; required delegated cgroup v2 CPU/memory/pids leaf; exact kill and empty-state verification; all partial/unsupported authority and admission fields false | Native synthetic primitive check, schema overclaim fixture, frozen profile/provenance, and later delegated-cgroup resource/lifecycle corpus | First checkpoint does not establish delegated resource or lifecycle controls. Linux remains unadmitted until every Tier A check passes on each claimed kernel/architecture |
