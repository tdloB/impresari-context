...
| SEC-T-052 | Linux primitive or cgroup overclaim | Kernel API presence, a partial Landlock/seccomp probe, controller inventory, a writable cgroup path, or a privileged setup process is presented as complete per-job OS confinement | Closed component profiles/receipts; `no_new_privs`; exact read-only staged input; architecture-pinned default-deny seccomp; descriptor closure; zero writable path-backed filesystem; one transient delegated subtree; unprivileged supervisor; atomic `CLONE_INTO_CGROUP`; frozen CPU/memory/pids leaves; exact kill and empty-state verification; overall confinement and admission remain false in component receipts | Native synthetic primitive check; CI-only delegated resource/lifecycle check; schema overclaim fixtures; frozen profiles/provenance; later composite and multi-host corpus | Component success is not composite IAR-1B admission. CI sudo is limited to transient-service creation; Linux remains unadmitted until every Tier A check passes on each claimed kernel/architecture |
