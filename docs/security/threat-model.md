...
| SEC-T-052 | Linux primitive, cgroup, or composition overclaim | Kernel API presence, a partial Landlock/seccomp probe, controller inventory, a writable cgroup path, separate component passes, or a privileged setup process is presented as complete per-job OS confinement | Closed profiles/receipts; `no_new_privs`; exact read-only staged input; architecture-pinned default-deny seccomp; descriptor closure; zero writable path-backed filesystem; one transient delegated subtree; unprivileged supervisor; profile limits applied before atomic `CLONE_INTO_CGROUP`; primitive denials reproduced inside that exact worker; exact kill and empty-state verification; production admission remains false | Native synthetic primitive check; CI-only delegated component check; single-service composite check; schema overclaim fixtures; frozen profiles/provenance; later multi-host corpus | CI sudo is limited to transient-service creation. An exact-host composite candidate is not a multi-target or production admission, and no component or candidate evidence authorizes real analyzer execution |
