...
| SEC-T-051 | macOS XPC sandbox or lifecycle overclaim | An entitlement mistake, broad grant, XPC substitution, network path, unrelated-resource access, retained container, unbounded reply, or incomplete resource/process-tree control is presented as a production sandbox | Closed synthetic request/receipt; ad hoc nested signing; effective entitlement inspection; no network entitlement; external file, credential, unrelated-process, and live loopback denials; bounded request/reply; all admission and authority fields fixed false; documented resource/lifecycle rejection | Native synthetic App Sandbox/XPC check plus closed schema, valid partial fixture, invalid overclaim fixture, provenance hashes, and platform-interface review | Hard per-job memory, process-count/tree, and deterministic teardown are unavailable under the selected documented architecture; macOS remains at IAR-1A and is not admitted at IAR-1B |
