...
## ADR-0104 Retained Engine Candidate Boundary
...
exact public source + patch + lock + toolchain + build-image digest
 ...
 yr + checksums + dependency closure + SBOM + provenance + dispositions
                              |
                              v
 one authenticated-reader seven-day Actions artifact (unadmitted)
                              |
                              v
      separate hash/schema/member verification without execution
```

The implemented build job has no free-form inputs and no repository, package,
...
## ADR-0105 Ephemeral Reproducibility Diagnostic

```text
one verified source archive + frozen patch + one locked dependency fetch
                              |
                              v
       baseline A/B clean builds + canonical A/B clean builds
                              |
                              v
       four SHA-256 identities + one closed source-free result
                              |
                              v
          mandatory deletion; no execution and no upload
```
...
## ADR-0106 Approved Source-Only Production Ruleset Boundary
...
substituted with AI review. The approved Option A implementation freezes three
original observation rules and twelve generated non-malicious fixtures. It
creates no compilation, analyzer-execution, artifact, repository-scan,
production, detection-quality, safety, or IAR-2 authority.
