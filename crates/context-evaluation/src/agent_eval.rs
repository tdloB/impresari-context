...
pub fn validate_records(spec: &StudySpec, records: &[RunRecord]) -> Result<(), String> {
    ...
    for (index, record) in records.iter().enumerate() {
        ...
        validate_usage(&record.agent_usage)?;
        if let Some(packet_usage) = &record.packet_usage {
            validate_usage(packet_usage)?;
        }
        ...
        verify_evidence(&source_root, expected_task, &record.evidence)?;
        if !record.evidence_verified {
            return Err(format!(
                "run {} did not satisfy evidence verification",
                record.sequence
            ));
        }
    }
...
pub fn write_markdown(path: &Path, summary: &EvaluationSummary) -> Result<(), String> {
    let mut text = format!(
        "# Agent-context evaluation: {}\n\nRuns: {}\n\n| Arm | Correctness | Evidence verified | Mean tokens | Mean tool calls | Mean repo reads | Mean wall-clock (ms) |\n| --- | ---: | ---: | ---: | ---: | ---: | ---: |\n",
    ...
    writeln!(
        text,
        "\nTreatment versus mean baseline: {:.1}% fewer tokens, {:.1}% fewer tool calls, {:.1}% fewer repository reads, {:.1}% faster; correctness delta {:+.1} points; evidence-verification delta {:+.1} points.\n",
...
fn execute_run(
    ...
    if arm == Arm::Treatment {
        let request = AdapterRequest {
            ...
            model_identifier: &spec.execution.model_identifier,
            container_image: &spec.execution.container_image,
            turn_limit: spec.execution.turn_limit,
            packet: None,
        };
        let (response, elapsed) =
            execute_adapter::<PacketResponse, _>(spec, &spec.packet_command, &request)?;
    ...
    let request = AdapterRequest {
        ...
        container_image: &spec.execution.container_image,
        turn_limit: spec.execution.turn_limit,
        packet: packet.as_deref(),
    };
    let (response, agent_wall_clock_millis) =
        execute_adapter::<AgentResponse, _>(spec, &spec.agent_command, &request)?;
    if response.source_fingerprint_sha256 != fingerprint_before {
        return Err(format!(
    ...
    let evidence_verified = verify_evidence(source_root, task, &response.evidence).is_ok();
...
fn verify_evidence(
    ...
    for required in &task.required_evidence {
        ...
        let bytes = source_range(root, required)?;
        if citation.sha256 != hash_bytes(&bytes) {
            return Err(format!(
                "task {} cited modified or incorrect evidence {}",
                task.id, required.path
            ));
        }
    }
...
fn source_range(root: &Path, requirement: &EvidenceRequirement) -> Result<Vec<u8>, String> {
    let path = resolve_source_file(root, &requirement.path)?;
    let contents = fs::read_to_string(&path)
        .map_err(|error| format!("read evidence {}: {error}", requirement.path))?;
    let start = usize::try_from(requirement.line_start.saturating_sub(1))
        .map_err(|_| "invalid evidence line".to_owned())?;
    let end =
        usize::try_from(requirement.line_end).map_err(|_| "invalid evidence line".to_owned())?;
    let lines = contents.lines().collect::<Vec<_>>();
    if start >= lines.len() || end > lines.len() {
        return Err(format!("evidence range outside file: {}", requirement.path));
    }
...
fn summarize_arm(records: &[RunRecord], arm: Arm) -> ArmSummary {
    ...
    ArmSummary {
        ...
        mean_tool_calls: mean(
            arm_records
                .iter()
                .map(|record| record.total_tool_calls as f64),
        ),
        mean_repository_file_reads: mean(
            arm_records
                .iter()
                .map(|record| record.total_repository_file_reads as f64),
        ),
