...
#[test]
fn macos_local_vm_guest_supply_chain_profile_and_fixture_provenance_are_frozen() {
    let root = repository_root();
    let profile_path = root.join("profiles/v1/iar-macos-local-vm-guest-supply-chain-v1.json");
    let profile_bytes = fs::read(&profile_path).expect("macOS local VM supply-chain profile");
    let sidecar = fs::read_to_string(
        root.join("profiles/v1/iar-macos-local-vm-guest-supply-chain-v1.sha256"),
    )
    .expect("macOS local VM supply-chain profile digest sidecar");
    assert_eq!(
        lowercase_hex(Sha256::digest(&profile_bytes)),
        sidecar.split_whitespace().next().expect("profile digest")
    );
    assert_eq!(
        ...
        fs::read(
            root.join(
                "tests/conformance/v1/valid/iar-macos-local-vm-guest-supply-chain-profile.json"
            )
        )
        .expect("macOS local VM supply-chain profile fixture")
    );
    let provenance = read_json(
        &root
            .join("tests/conformance/v1/macos-local-vm-guest-supply-chain-fixture-provenance.json"),
    );
    assert_eq!(
        provenance["review_status"],
        "approved_original_synthetic_only"
    ...
    let provenance_paths = provenance["cases"]
        .as_array()
        .expect("macOS local VM supply-chain provenance cases")
        .iter()
        .map(|case| {
            assert_eq!(case["origin"], "original_synthetic");
            assert_eq!(case["license"], "Apache-2.0");
    ...
    let declared_paths = conformance["cases"]
        ...
        .filter(|case| {
            matches!(
                case["schema"].as_str().expect("schema").split('#').next(),
                Some(
                    "macos-local-vm-guest-release-manifest.schema.json"
                        | "macos-local-vm-guest-supply-chain-profile.schema.json"
                        | "macos-local-vm-guest-supply-chain-receipt.schema.json"
                )
            )
        })
        .map(|case| case["fixture"].as_str().expect("fixture"))
...
#[test]
fn macos_local_vm_upstream_auth_profile_and_fixture_provenance_are_frozen() {
    let root = repository_root();
    let profile_path = root.join("profiles/v1/iar-macos-local-vm-upstream-auth-v1.json");
    let profile_bytes = fs::read(&profile_path).expect("macOS local VM upstream-auth profile");
    let sidecar =
        fs::read_to_string(root.join("profiles/v1/iar-macos-local-vm-upstream-auth-v1.sha256"))
            .expect("macOS local VM upstream-auth profile digest sidecar");
    assert_eq!(
        lowercase_hex(Sha256::digest(&profile_bytes)),
        sidecar.split_whitespace().next().expect("profile digest")
    );
    assert_eq!(
        profile_bytes,
        fs::read(
            root.join("tests/conformance/v1/valid/iar-macos-local-vm-upstream-auth-profile.json")
        )
        .expect("macOS local VM upstream-auth profile fixture")
    );
    let provenance = read_json(
        &root.join("tests/conformance/v1/macos-local-vm-upstream-auth-fixture-provenance.json"),
    );
    assert_eq!(
        provenance["review_status"],
        "approved_original_metadata_with_provider_evidence"
    );
    for prohibited in [
        "contains_executable_artifacts",
        "contains_malware_or_live_signatures",
    ...
    assert_eq!(provenance["network_or_provider_data_used"], true);
    let provenance_paths = provenance["cases"]
        .as_array()
        .expect("macOS local VM upstream-auth provenance cases")
        .iter()
        .map(|case| {
            assert_eq!(case["origin"], "original_metadata");
            assert_eq!(case["license"], "Apache-2.0");
            let relative = case["path"].as_str().expect("fixture path");
            assert!(!relative.starts_with('/') && !relative.contains(".."));
            let bytes = fs::read(root.join("tests/conformance/v1").join(relative))
    ...
    let declared_paths = conformance["cases"]
        ...
        .filter(|case| {
            matches!(
                case["schema"].as_str().expect("schema").split('#').next(),
                Some(
                    "macos-local-vm-upstream-auth-record.schema.json"
                        | "macos-local-vm-upstream-auth-profile.schema.json"
                        | "macos-local-vm-upstream-auth-receipt.schema.json"
                )
            )
        })
        .map(|case| case["fixture"].as_str().expect("fixture"))
...
#[test]
fn macos_local_vm_vulnerability_review_profile_and_fixture_provenance_are_frozen() {
    let root = repository_root();
    let profile_path = root.join("profiles/v1/iar-macos-local-vm-vulnerability-review-v1.json");
    let profile_bytes = fs::read(&profile_path).expect("macOS local VM vulnerability profile");
    let sidecar = fs::read_to_string(
        root.join("profiles/v1/iar-macos-local-vm-vulnerability-review-v1.sha256"),
    )
    .expect("macOS local VM vulnerability profile digest sidecar");
    assert_eq!(
        lowercase_hex(Sha256::digest(&profile_bytes)),
        sidecar.split_whitespace().next().expect("profile digest")
    );
    assert_eq!(
        profile_bytes,
        fs::read(root.join(
            "tests/conformance/v1/valid/iar-macos-local-vm-vulnerability-review-profile.json"
        ))
        .expect("macOS local VM vulnerability profile fixture")
    );
    let provenance =
        read_json(&root.join(
            "tests/conformance/v1/macos-local-vm-vulnerability-review-fixture-provenance.json",
        ));
    assert_eq!(
        provenance["review_status"],
        "approved_original_metadata_with_provider_evidence"
    ...
    let provenance_paths = provenance["cases"]
        .as_array()
        .expect("macOS local VM vulnerability provenance cases")
        .iter()
        .map(|case| {
            assert_eq!(case["origin"], "original_metadata");
            assert_eq!(case["license"], "Apache-2.0");
    ...
    let declared_paths = conformance["cases"]
        ...
        .filter(|case| {
            matches!(
                case["schema"].as_str().expect("schema").split('#').next(),
                Some(
                    "macos-local-vm-vulnerability-assessment-record.schema.json"
                        | "macos-local-vm-vulnerability-review-profile.schema.json"
                        | "macos-local-vm-vulnerability-review-receipt.schema.json"
                )
            )
        })
        .map(|case| case["fixture"].as_str().expect("fixture"))
